...
let i = '9';
let j = '9';
console.log(i + j == 99);     // false  ('9' + '9' = '99')
console.log(i + j === '99');  // true
console.log(i + j === 99);    // false  (型別不同)

var k = 3;
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21);   // java script 會將l轉型為數字型別, k * l = 24 > 21
...
const gymCoach = {
  'gymName':'高雄市健身教練聯盟',
  'city':'高雄市',
  'description': '專注於提供高品質的健身指導服務。健身房內有多位專業教練',
  'coaches':[
    {
      'coachName': '王教練',
      'skills': ['力量訓練', '減重課程'],
...
let courseRevenueTarget = 50000
// 練習：第一位教練（可將下方程式碼註解移除，完成答題）
if (performanceData.coaches[0].performance < 50000) {
  // 請填寫第一位教練業績增長程式碼，使用 +=
  performanceData.coaches[0].performance += courseRevenueTarget - performanceData.coaches[0].performance
}

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）
if (performanceData.coaches[1].performance < 50000) {
  // 請填寫第一位教練業績增長程式碼，使用 +=
  performanceData.coaches[1].performance += courseRevenueTarget - performanceData.coaches[1].performance
}
...
*/
const activities = {
  treadmill: {caloriesPerMinute: 10},
  yoga: {caloriesPerMinute: 5},
...
calorieBurn += 
  ...
  activities.treadmill.caloriesPerMinute * 30 + 
...
let heartRateLimit = 130
// 範例：週一
// if (exerciseRecords[0].duration >= durationLimit && exerciseRecords[0].heartRate >= heartRateLimit) {
//   totalDuration += mondayDuration;
//   validDays += 1;
// }
// 使用 forEach 的方式來計算
exerciseRecords.forEach(records => {
  if (records.duration >= durationLimit && records.heartRate >= heartRateLimit) {
    totalDuration += records.duration;
    validDays += 1;
  }

});

console.log('有效運動天數:' + validDays)
// 練習：週二、週三、週四、週五、週六
